import AppEndpoint from "./appEndpoint";
import AppURL from "./appURL";
import LenspostURL from "./lenspostURL";


// Environments
export const ENV = process.env.NODE_ENV;

// Environment Variables
export const WALLETCONNECT_KEY = process.env.WALLETCONNECT_KEY;

// Application
export const APP_NAME = "Mint Lenspost";
export const DESCRIPTION = `${APP_NAME} is a platform for minting NFTs.`;
export const AUTHOR = "Lenspost";
export const APP_URL = AppURL[ENV as keyof typeof AppURL];

// Lenspost
export const LENSPOST_APP_NAME = "Lenspost";
export const LENSPOST_DESCRIPTION = `${LENSPOST_APP_NAME} is a fun onchain 'canva' that turns NFT holders into content creators with one click drag-drop-remix! Make NFTs do more for you as you churn out DOPE memes, gifs, social content & more! The most fun way to permissionlessly collaborate, monetize & even split revenues across chains. We're NFT INFRA at the back, RAVE party in the front - powering co-creation, revenue share & social distribution with BIG MEME ENERGY!`;
export const LENSPOST_APP_URL = LenspostURL[ENV as keyof typeof LenspostURL];


// Social
export const LENSPOST_TWITTER_USERNAME = "@Lenspostxyz";

// Zora
export const ZORA_REWARD_FEE = "0.000777";

// Addresses
export const LENSPOST_ETH_ADDRESS = "0x77fAD8D0FcfD481dAf98D0D156970A281e66761b";
export const LENSPOST_SOLANA_ADDRESS = "2PsV6hNEUc3rSMGqKcHTnRBemaWBQX3dYgUqVtEFxkwa";

// URLs
export const BACKEND_ENDPOINT = AppEndpoint[ENV as keyof typeof AppEndpoint];